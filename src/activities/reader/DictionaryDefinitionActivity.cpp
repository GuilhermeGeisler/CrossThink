...
void DictionaryDefinitionActivity::wrapText() {
  ...
  if (ESP.getMaxAllocHeap() < 18 * 1024) {
    ...
    if (!isContinuingFromSilentReboot()) {
      LOG_INF("DICT", "wrapText: maxAlloc=%u too low (< 18 KB); triggering silent restart with OpenDefinition('%s')",
    ...
    lineHeight = renderer.getLineHeight(UI_12_FONT_ID);
    linesPerPage = 1;
    maxScroll = 0;
    return;
  }
  ...
  while (std::getline(ss, paragraph, '\n')) {
    ...
    auto pLines = renderer.wrappedText(UI_12_FONT_ID, paragraph.c_str(), maxWidth, 1000);
...
void DictionaryDefinitionActivity::render(RenderLock&&) {
  ...
  if (isLoading) {
    ...
    renderer.drawText(UI_12_FONT_ID, margin, currentY, notFoundMsg.c_str());

    currentY += renderer.getLineHeight(UI_12_FONT_ID) * 2;
    renderer.drawText(UI_12_FONT_ID, margin, currentY, tr(STR_PRESS_CONFIRM_SUGGESTIONS));
  } else {
    ...
    for (int i = 0; i < linesToDraw; ++i) {
      renderer.drawText(UI_12_FONT_ID, margin, currentY, wrappedLines[scrollOffset + i].c_str());
      currentY += lineHeight;
    }

    if (scrollOffset > 0) {
      renderer.drawText(UI_12_FONT_ID, renderer.getScreenWidth() - margin - 20, margin * 2, "^");
    }
    if (scrollOffset < maxScroll) {
      renderer.drawText(UI_12_FONT_ID, renderer.getScreenWidth() - margin - 20, renderer.getScreenHeight() - 60, "v");
    }
  }
