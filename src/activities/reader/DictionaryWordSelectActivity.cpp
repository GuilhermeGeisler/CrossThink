...
// Borrowed from CrumBLE's BookmarkStore — max preview length for context snippets
static constexpr size_t BOOKMARK_PREVIEW_MAX = 1024;
...
void DictionaryWordSelectActivity::openDefinitionOverlay(const std::string& word) {
  ...
  if (!defOverlayCaptureValid_) {
    LOG_INF("DICT",
            "openDefinitionOverlay: storeBwBuffer failed; overlay will repaint via word-select re-render on dismiss");
  }
  defOverlay_ = true;
...
void DictionaryWordSelectActivity::wrapDefinition(const std::string& definition) {
  ...
  while (std::getline(ss, paragraph, '\n')) {
    ...
    auto pLines = renderer.wrappedText(UI_12_FONT_ID, paragraph.c_str(), popupMaxWidth, 1000);
...
void DictionaryWordSelectActivity::renderDefinitionOverlay() {
  ...
  if (defOverlayLoading_) {
    renderer.drawText(UI_12_FONT_ID, textX, currentY, tr(STR_LOOKING_UP));
  } else if (defOverlayLowMemory_) {
    renderer.drawText(UI_12_FONT_ID, textX, currentY, defTargetWord_.c_str(), EpdFontFamily::BOLD);
    currentY += renderer.getLineHeight(UI_12_FONT_ID) * 2;
    renderer.drawText(UI_12_FONT_ID, textX, currentY, "Low memory -- back out");
    currentY += renderer.getLineHeight(UI_12_FONT_ID);
    renderer.drawText(UI_12_FONT_ID, textX, currentY, "and reopen Lookup");
  } else if (defOverlayNotFound_) {
    renderer.drawText(UI_12_FONT_ID, textX, currentY, defTargetWord_.c_str(), EpdFontFamily::BOLD);
    currentY += renderer.getLineHeight(UI_12_FONT_ID) * 2;
    std::string msg = std::string(tr(STR_WORD_NOT_FOUND)) + defTargetWord_;
    renderer.drawText(UI_12_FONT_ID, textX, currentY, msg.c_str());
  } else {
    renderer.drawText(UI_12_FONT_ID, textX, currentY, defTargetWord_.c_str(), EpdFontFamily::BOLD);
    const int titleH = renderer.getLineHeight(UI_12_FONT_ID);
    const int titleWidth = renderer.getTextWidth(UI_12_FONT_ID, defTargetWord_.c_str(), EpdFontFamily::BOLD);
    ...
    for (int i = 0; i < linesToDraw; ++i) {
      renderer.drawText(UI_12_FONT_ID, textX, currentY, defLines_[defScrollOffset_ + i].c_str());
      currentY += lineHeight;
    }
    if (defScrollOffset_ > 0) {
      renderer.drawText(UI_12_FONT_ID, popupX + popupW - kPopupPadding - 12, popupY + kPopupPadding, "^");
    }
    if (defScrollOffset_ < defMaxScroll_) {
      renderer.drawText(UI_12_FONT_ID, popupX + popupW - kPopupPadding - 12,
