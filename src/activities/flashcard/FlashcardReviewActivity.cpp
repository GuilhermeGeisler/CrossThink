...
static void wrapByCodepoint(GfxRenderer& r, int fontId, const std::string& seg, int maxWidth,
...
static void wrapSegment(GfxRenderer& r, int fontId, const std::string& seg, int maxWidth,
                        std::vector<std::string>& out) {
  if (seg.empty()) {
    out.push_back("");
    return;
...
static std::vector<std::string> wrapText(GfxRenderer& r, int fontId, const std::string& text, int maxWidth) {
...
static int pickMainFont(GfxRenderer& r, const std::string& text, int maxWidth, int availH,
...
void FlashcardReviewActivity::onEnter() {
  ...
  reviewQueue = deck.buildReviewQueue(today, PET_SETTINGS.flashcardNewPerDay, PET_SETTINGS.flashcardMaxReviewPerDay);
  currentQueueIndex = 0;
...
void FlashcardReviewActivity::renderFront(const FlashcardCard& card) {
  ...
  int mainFont = pickMainFont(renderer, mainText, contentWidth, contentHeight - hintReserve, mainLines);
...
void FlashcardReviewActivity::renderBack(const FlashcardCard& card) {
  ...
  snprintf(lbl1, sizeof(lbl1), "%s %s", tr(STR_FLASHCARD_AGAIN), FlashcardSrs::previewInterval(srs, SrsRating::Again));
  snprintf(lbl2, sizeof(lbl2), "%s %s", tr(STR_FLASHCARD_EASY), FlashcardSrs::previewInterval(srs, SrsRating::Easy));
  snprintf(lbl3, sizeof(lbl3), "%s %s", tr(STR_FLASHCARD_HARD), FlashcardSrs::previewInterval(srs, SrsRating::Hard));
  snprintf(lbl4, sizeof(lbl4), "%s %s", tr(STR_FLASHCARD_GOOD), FlashcardSrs::previewInterval(srs, SrsRating::Good));
